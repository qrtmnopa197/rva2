...
rva2_wrangle_psychopy_data <- function(csv_path){
  if(!is.function(get_csvs_to_analyze)){
    stop("get_csvs_to_analyze() is not loaded into the environment of rva2_wrangle_psychopy_data(). This function is in s22_utilities.R.")
  }
  ...
  trials <- select(df_full, any_of(c("outcome", frac_img = "fA_img", val_rat = "val_slider_8.response", 
  ...
  trials$trial_sub <- c(1:nrow(trials)) #get the overall trial numbers for this subjects
